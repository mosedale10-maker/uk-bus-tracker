...
function liveFirstPotteriesGuidePath(filter = {}, gpsPoints = [], ping = null) {
  ...
  // First Potteries GPS is the authority for a recorded live stint; do not
  // snap a diversion back to the timetable alignment.
...
function popupHtml(bus, extra = {}, { omitStops = false, sidePanel = false } = {}) {
  ...
  const regAction = regLink
    ? `<div class="popup-reg-action-wrap">${fleetRegButtonHtml(reg, { ...regLinkOptions, className: "popup-reg-action" })}<span class="popup-reg-hint">Click above to use playback</span></div>`
    : reg
...
function staffPopup(item, extra = {}, { omitStops = false, sidePanel = false } = {}) {
  ...
  const regAction = regButton
    ? `<div class="popup-reg-action-wrap">${regButton}<span class="popup-reg-hint">Click above to use playback</span></div>`
    : staffReg
